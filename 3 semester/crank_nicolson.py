...
def thomas_algorithm_step(
    ...
    """
    Один шаг схемы Кранка–Николсона (метод прогонки).
    ...
    for j in range(1, n - 1):
        d_j = (
            u_prev[j]
            + TAU * source_term(x[j], t)
            + (TAU * A_COEFF / (2 * H**2)) * (u_prev[j + 1] - 2 * u_prev[j] + u_prev[j - 1])
        )
        beta[j] = (c_coef * beta[j - 1] + d_j) / (b_coef - c_coef * alpha[j - 1])
        alpha[j] = a_coef / (b_coef - c_coef * alpha[j - 1])
...
def main() -> None:
    ...
    print(f"Порядок аппроксимации: O(tau^2 + h^2) = O({H**2 + TAU**2})")
